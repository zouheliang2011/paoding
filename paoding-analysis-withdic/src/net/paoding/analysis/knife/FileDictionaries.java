...
public class FileDictionaries implements Dictionaries {
	 ...
	 * @return
	 */
	public synchronized Dictionary getVocabularyDictionary(String itemcat) {
		if (vocabularyDictionary == null) {
			...
			vocabularyDictionary = new HashBinaryDictionary(
					getVocabularyWords(itemcat), 0x2fff, 0.75f);
	...
	protected Word[] getVocabularyWords(String itemcat) {
		Map/* <String, Set<Word>> */dics =null;
		if(itemcat==null && itemcat.trim().equals(""))
		{
		dics = loadAllWordsIfNecessary();
		}else
		{
			dics = loadAllWordsIfNecessary(itemcat);
		}
		Set/* <Word> */set = null;
	...
	protected synchronized Map/* <String, Set<String>> */loadAllWordsIfNecessary(String itemcat) {
		if (allWords == null) {
			try {
				log.info("loading dictionaries from " + dicHome);
				allWords = FileWordsReader.readWords(dicHome, charsetName, maxWordLen,itemcat);
				if (allWords.size() == 0) {
					String message = "Not found any dictionary files, have you set the 'paoding.dic.home' right? ("
							+ this.dicHome + ")";
					log.error(message);
