...
public class TryPaodingAnalyzer {
	...
	private static void analysing(String itemcat) throws Exception {
		Analyzer analyzer;
		if(itemcat==null || itemcat.trim().equals(""))
		{
			System.out.println("请输入正确的分类编码，否则无法正确的匹配庖丁词库");
			return ;
		}
		if (analyzerName == null || analyzerName.length() == 0 || analyzerName.equalsIgnoreCase("paoding")) {
			...
			analyzer = new PaodingAnalyzer(properties,itemcat);
			if (mode != null) {
