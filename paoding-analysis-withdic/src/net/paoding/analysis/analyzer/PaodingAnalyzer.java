...
public class PaodingAnalyzer extends PaodingAnalyzerBean {
	...
	public static List<String> itemCat= new ArrayList<String>();
	/**
	 * 根据类路径下的paoding-analysis.properties构建一个PaodingAnalyzer对象
	 * <p>
	 * 在一个JVM中，可多次创建，而并不会多次读取属性文件，不会重复读取字典。
	 */
	public PaodingAnalyzer(String itemCat) {
		this(PaodingMaker.DEFAULT_PROPERTIES_PATH,itemCat);
	}

	/**
	 * @param propertiesPath null表示使用类路径下的paoding-analysis.properties
	 */
	public PaodingAnalyzer(String propertiesPath,String itemcat) {
		init(propertiesPath,itemcat);
	}

	protected void init(String propertiesPath,String itemcat) {
		// 根据PaodingMaker说明，
		...
		Paoding paoding = PaodingMaker.make(properties,itemcat);
		setKnife(paoding);
		setMode(mode);
	}
